...
begin
using NonlinearSolve,Plots, Latexify
...
# ╔═╡ e28e2cc8-b19d-4f22-a3f5-30286306176b
### Parameters

begin
	L = 0.1 		# bar length (m)
	ω = 2.0        # angular rate (rad/s)
end

# ╔═╡ 195816d9-1d17-430d-bf4b-0d8a62dd3fb7
### Variables
...
# ╔═╡ a2d15303-0a8e-4fb7-baaa-b9a092dbda54
### Constraint Function
begin
	function C_sym(q, t)
...
# ╔═╡ 9b947005-af45-477e-b734-3a499f959dc9
begin
    C_expr = C_sym(q_sym, t)
    Cq_sym = Symbolics.jacobian(C_expr, q_sym)
    Ct_sym = Symbolics.derivative.(C_expr, t)

    C_func  = eval(Symbolics.build_function(C_expr,  q_sym, t)[1])
    Cq_func = eval(Symbolics.build_function(Cq_sym, q_sym, t)[1])
    Ct_func = eval(Symbolics.build_function(Ct_sym, q_sym, t)[1])
end

# ╔═╡ 25ed378b-e795-4648-911f-6705349a46f3
### Velocity Function
begin
	function velocity(q, t)
    Cq = Cq_func(q, t)
    Ct = Ct_func(q, t)
    return -Cq \ Ct
	end
...
# ╔═╡ aac1e208-d16e-412f-9a5f-36c8f2418d05
### Acceleration Function
begin
	function acceleration(q, dq, t)
    h = 1e-6

    Cq = Cq_func(q, t)

    # Approximate time derivative of Jacobian
    Cq_t = (Cq_func(q .+ h*dq, t+h) - Cq) / h
    Ct_t = (Ct_func(q .+ h*dq, t+h) - Ct_func(q, t)) / h

    return Cq \ (-Cq_t * dq - Ct_t)
	end
end

# ╔═╡ c6c8cfeb-4b5b-4ae9-a7ca-8cc662a5b105
### Position Over Time
begin
	N = 100
	time_span = LinRange(0, 2π/ω, N)
	
	position_solution     = zeros(N, 9)
	velocity_solution     = zeros(N, 9)
	acceleration_solution = zeros(N, 9)
	
	position_guess = [-0.05, -0.05,  π/4,
	                   0.05, -0.05, -π/4,
	                   0.0,   0.0,   0.0]
	...
	for i in 1:length(time_span)
	    ...
	    prob = NonlinearProblem((q,p)->C_func(q,p), position_guess, t)
	    sol = solve(prob)
	
	    position_solution[i, :] .= sol.u
	
	    dq = velocity(sol.u, t)
	    velocity_solution[i, :] .= dq
	
	    ddq = acceleration(sol.u, dq, t)
	    acceleration_solution[i, :] .= ddq
	
	    position_guess .= sol.u
	end
end


# ╔═╡ 7b29d17c-858a-41a0-bcb5-1a25f77fb5f9
begin
    # Extract piston positions 
    slider1_position = position_solution[:, 1]   
    slider2_position = position_solution[:, 4]   

    # Extract piston velocities
    slider1_velocity = velocity_solution[:, 1]
    slider2_velocity = velocity_solution[:, 4]

    # Extract piston accelerations
    slider1_accel = acceleration_solution[:, 1]
    slider2_accel = acceleration_solution[:, 4]

    position_plot = plot(time_span, [slider1_position slider2_position],
             xlabel="t (s)", ylabel="s (m)",
             label=[L"s_1(t)" L"s_2(t)"],
             title="Slider Positions")

    velocity_plot = plot(time_span, [slider1_velocity slider2_velocity],
             xlabel="t (s)", ylabel=L"\dot{s}\,(m/s)",
             label=[L"\dot{s}_1(t)" L"\dot{s}_2(t)"],
             title="Slider Velocities")

    acceleration_plot = plot(time_span, [slider1_accel slider2_accel],
             xlabel="t (s)", ylabel=L"\ddot{s}\,(m/s^2)",
             label=[L"\ddot{s}_1(t)" L"\ddot{s}_2(t)"],
             title="Slider Accelerations")

    plot(position_plot, velocity_plot, acceleration_plot, 
         layout=(3,1), size=(900,900))
end

# ╔═╡ 00214eef-6d01-4f81-bdf6-c36ae37179f6
### Visualization
begin
	function plot_oriented_rectangle!(q, width, height; 
...
# ╔═╡ 6fec1f2a-7806-4c57-9f9f-90724cead74e
begin
	u_vals = range(0, 1, length=5)
	traj_x = [Float64[] for _ in u_vals]
	traj_y = [Float64[] for _ in u_vals]
	
	@gif for i in 1:length(time_span)
	
	    q = position_solution[i, :]
	    x1, y1, θ1 = q[1], q[2], q[3]
	    x2, y2, θ2 = q[4], q[5], q[6]
	    x3, y3, θ3 = q[7], q[8], q[9]
	
	    # Updates the ellipse trajectories
	    ...
	    # Base plot
	    plt = plot(xlim=(-0.15,0.15), ylim=(-0.15,0.15),
	               aspect_ratio=:equal, legend=false,
	               title="Combined Motion + Ellipse Trajectories")
	
	    # Tracks
	    plot!(plt, [-0.2, 0.2], [-0.2, 0.2], lw=2, color=:black)
	    plot!(plt, [-0.2, 0.2], [ 0.2,-0.2], lw=2, color=:black)
	
	    # Ellipse trails
	    for j in eachindex(u_vals)
	        plot!(plt, traj_x[j], traj_y[j], lw=2, alpha=0.6)
	    end
	
	    # Rigid Bodies
	    plot_oriented_rectangle!([x1, y1, θ1], 0.03, 0.015)
	    plot_oriented_rectangle!([x2, y2, θ2], 0.03, 0.015)
	    plot_oriented_rectangle!([x3, y3, θ3], 0.01, 0.1)
	
	    plt
	end
end

# ╔═╡ 546698b8-3b2d-451f-87eb-87032b5a8037
begin
	function mechanism_snapshot(i)
	    q = position_solution[i, :]
	    ...
	    frame = plot([-0.14, 0.14], [-0.14, 0.14],
	        ...
	        aspect_ratio = :equal,
	        xlims = (-0.14, 0.14), ylims = (-0.14, 0.14),
	        title = "t = $(round(time_span[i], digits = 2)) s",
	        label = "+45° Track")
	
	    plot!(frame, [-0.14, 0.14], [0.14, -0.14],
	          lw = 2, label = "-45° Track")
	
	    plot!(frame, [x1, x2], [y1, y2],
	          lw = 4, label = "Rigid bar")
	
	    scatter!(frame, [x1, x2], [y1, y2],
	             ms = 6, label = ["Slider 1" "Slider 2"])
	
	    return frame
	end
end

# ╔═╡ 7cb3e68b-975c-49cc-bff9-215093386d50
begin
	snapshot_indices = round.(Int, range(1, length(time_span), length=6))
	
	plot([mechanism_snapshot(i) for i in snapshot_indices]...,
...
manifest_format = "2.0"
project_hash = "4dfeb23c4c804522e179a37e577d1e7460519652"
...
git-tree-sha1 = "adbd4c44ef9361c67dd5dc192b66ac12b9e1be85"
...
git-tree-sha1 = "26e80b68bc6b8dae0a09db4ebae28aead9b58255"

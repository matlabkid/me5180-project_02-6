...
# ╔═╡ 8e61e635-11a0-43d5-aed2-48b8bb14f929
using LinearAlgebra

# ╔═╡ 9210f417-ae1d-46ec-ac6e-777b95eeccd6
using NonlinearSolve

# ╔═╡ 55a2031e-0084-40a0-813d-7de05070c5e7
using Plots
...
# ╔═╡ b8af066f-6877-437c-a783-dbebe4dec6f1
# Geometry
L = 0.10              # [m]

# ╔═╡ f8ffb0e4-2761-4a10-9974-56be0af436f2
a = L / 2

# ╔═╡ 2970f685-1dae-4a84-a4a3-d274acf00f6a
# Prescribed motion
ω = 2.0               # rad/s

# ╔═╡ 48b529f2-302c-47c7-83e3-9c356af3c5fe
# Time
t0 = 0.0

# ╔═╡ b0ae3603-7726-4079-a587-969f44900715
tf = π                # one full rotation

# ╔═╡ 65e4bce0-29f8-4e49-908d-d9e13c5856fb
N  = 300

# ╔═╡ c595c57c-9e28-415e-a5f5-b075164e6203
ts = range(t0, tf, length=N)

# ╔═╡ 1d4edf83-33ee-4d32-9aaf-8fd6b134cbfb
# Generalized coordinates ordering
# q = [x1,y1,θ1,  x2,y2,θ2,  x3,y3,θ3]
x1,y1,θ1 = 1,2,3

# ╔═╡ 2afa9f98-0974-4d8b-a4f8-443c8aaa53ae
x2,y2,θ2 = 4,5,6

# ╔═╡ 461b0b06-bc25-4416-85b1-f78f2d91b8cf
x3,y3,θ3 = 7,8,9

# ╔═╡ 3799ccf3-12f4-43b7-a7fd-f221c18e5a67
function constraints!(C, q, p)
    t = p   # treat parameter as time
...
# ╔═╡ 52da31d4-8b89-47a4-842f-f000e0adc43b
function constraint_jacobian(q)
    ...
    Cq[6,x1] = 1; Cq[6,x3] = -1; Cq[6,θ3] = -a*s
    Cq[7,y1] = 1; Cq[7,y3] = -1; Cq[7,θ3] =  a*c

    Cq[8,x2] = 1; Cq[8,x3] = -1; Cq[8,θ3] =  a*s
    Cq[9,y2] = 1; Cq[9,y3] = -1; Cq[9,θ3] = -a*c

    return Cq
end


# ╔═╡ 1a267e67-f3df-4276-a2ed-54ec1c59098f
Ct() = vcat(-ω, zeros(8))

# ╔═╡ ac9b9c40-3f72-4732-b105-57175277358c
function acceleration_rhs(q, qd)
    rhs = zeros(9)
...
# ╔═╡ 0a9ca9ac-7caa-4634-86dd-f6edcb59142f
# Initial guess
q0 = zeros(9)

# ╔═╡ 22b7ea97-112a-4bd3-8536-121bd0697b21
q0[θ1] = -π/4

# ╔═╡ 65e9489f-64d7-4edd-b515-532ed847e350
q0[θ2] =  π/4

# ╔═╡ fa510efa-6d04-4e25-b83a-8702a0605cf3
Q   = zeros(9, N)

# ╔═╡ 823308eb-17e6-4979-b336-bcdcfbf8befb
Qd  = zeros(9, N)

# ╔═╡ ffd18e28-2181-43c4-8eba-fc1b538c3d11
Qdd = zeros(9, N)

# ╔═╡ f9d9dbe0-6261-46c5-afdd-289efa2bc3ed
let q_guess = copy(q0)
    for (i,t) in enumerate(ts)
        prob = NonlinearProblem(constraints!, q_guess, t)
        sol  = solve(prob, NewtonRaphson())
        q    = sol.u

        Cq  = constraint_jacobian(q)
        qd  = -Cq \ Ct()
        qdd = -Cq \ acceleration_rhs(q, qd)
...
# ╔═╡ 16a08664-4d0b-424d-8ead-80da7f3f6730
plot(Q[x1,:], Q[y1,:],
     aspect_ratio=1,
     label="Piston 1")

# ╔═╡ b9a1a9ca-0c40-48f0-9a51-a3d27aa980b7
plot!(Q[x2,:], Q[y2,:],
      label="Piston 2")

# ╔═╡ efed862b-d5e7-45c4-85b7-c01ad5251726
anim = @animate for i in 1:size(Q, 2)
    q = Q[:, i]
    ...
    # ---- MISSING DIAGONAL (NOW ADDED) ----
    plot!([-0.15, 0.15], [0.15, -0.15],   # −45° track
          color = :black, lw = 1)
...
# ╔═╡ a8d90768-c60f-47cc-b52f-e8ff700edd7d
...
manifest_format = "2.0"
project_hash = "4728e4fed764206cef1101ea0ca25e0da6d009d4"

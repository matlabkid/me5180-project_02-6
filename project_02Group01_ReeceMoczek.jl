...
using Symbolics, LinearAlgebra, DifferentialEquations, Plots, LaTeXStrings
...
# ╔═╡ ef3bf1ef-6444-45de-9419-e9d823ac86b9

md"""
# 1. Determine the Constraint equations  $C(\mathbf{q},t)$
...
$(\mathbf{r}_2-\mathbf{r}_1)^T(\mathbf{r}_2-\mathbf{r}_1)=L^2$

**Final:**

$C(\mathbf{q},t)=s_1^2+s_2^2-L^2=0$
"""
#\\Reece Moczek 4/4/26

# ╔═╡ 16444d12-76c3-4a8f-ac71-920d82bc2d69

md"""
# 2. Solve for Velocities and Accelerations

## Constraint (vector form)
...
## Velocity level (differentiate once)
$\begin{aligned}
\dot C_1 &: \;\cos135^\circ\,\dot s_2 - \cos45^\circ\,\dot s_1 + L\sin\theta_3\,\dot\theta_3 = 0\\
\dot C_2 &: \;\sin135^\circ\,\dot s_2 - \sin45^\circ\,\dot s_1 - L\cos\theta_3\,\dot\theta_3 = 0
\end{aligned}$

With the driver $\dot\theta_3=\omega$ (given), solve the 2×2 linear system for $\dot s_1,\dot s_2.$

---

## Acceleration level (differentiate again)
Because $\ddot\theta_3=0$ and $\dot\theta_3=\omega$ is constant,
$\begin{aligned}
\ddot C_1 &: \;\cos135^\circ\,\ddot s_2 - \cos45^\circ\,\ddot s_1 + L\cos\theta_3\,\omega^2 = 0\\
\ddot C_2 &: \;\sin135^\circ\,\ddot s_2 - \sin45^\circ\,\ddot s_1 + L\sin\theta_3\,\omega^2 = 0
\end{aligned}$

Solve the 2×2 linear system for $\ddot s_1,\ddot s_2.$
"""
#\\Reece Moczek 4/4/26

# ╔═╡ db6d278b-3c86-4f70-a790-98f94068dd56

function slider_vel_acc(bar_angle; bar_length=0.10, bar_angular_rate=2.0)
    cos45 = √2/2;  sin45 = √2/2
    cos135 = -√2/2; sin135 = √2/2
    A = [-cos45  cos135;
         -sin45  sin135]

    velocity_rhs = [-bar_length*sin(bar_angle)*bar_angular_rate;
                     bar_length*cos(bar_angle)*bar_angular_rate]
    acceleration_rhs = [-bar_length*cos(bar_angle)*bar_angular_rate^2;
                        -bar_length*sin(bar_angle)*bar_angular_rate^2]

    slider_velocity = A \ velocity_rhs          # [s1_dot, s2_dot]
    slider_accel    = A \ acceleration_rhs      # [s1_ddot, s2_ddot]
    return slider_velocity[1], slider_velocity[2], slider_accel[1], slider_accel[2]
end
#\\Reece Moczek 4/4/26
...
begin
	bar_angle = 0.8
	s1_dot, s2_dot, s1_ddot, s2_ddot = slider_vel_acc(bar_angle)
end
#\\Reece Moczek 4/4/26
...
md"""
# 3. One Full Rotation Plots

With the bar angle driven by:
$$\theta_3(t)=\omega t,\qquad \omega=2\ \text{rad/s}$$

Slider positions (from geometry + constraints):
$s_1(t)= -\frac{L}{\sqrt{2}}\big(\cos\theta_3+\sin\theta_3\big),\qquad
s_2(t)= \frac{L}{\sqrt{2}}\big(\sin\theta_3-\cos\theta_3\big)$

Velocities (differentiate once, using $\dot\theta_3=\omega$):
$\dot s_1(t)= -\frac{L\omega}{\sqrt{2}}\big(\cos\theta_3-\sin\theta_3\big),\qquad
\dot s_2(t)= \frac{L\omega}{\sqrt{2}}\big(\sin\theta_3+\cos\theta_3\big)$

Accelerations (differentiate again, using $\ddot\theta_3=0$):
$\ddot s_1(t)= \frac{L\omega^2}{\sqrt{2}}\big(\sin\theta_3+\cos\theta_3\big),\qquad
\ddot s_2(t)= \frac{L\omega^2}{\sqrt{2}}\big(\cos\theta_3-\sin\theta_3\big)$
"""
...
begin
    bar_length = 0.10
    bar_angular_rate = 2.0
    time_end = 2π / bar_angular_rate
    time_vector = range(0, time_end, length=500)
    bar_angle_vector = bar_angular_rate .* time_vector

    scale = bar_length / √2
    slider1_position = .-scale .* (cos.(bar_angle_vector) .+ sin.(bar_angle_vector))
    slider2_position =  scale .* (sin.(bar_angle_vector) .- cos.(bar_angle_vector))

    scale_v = bar_length * bar_angular_rate / √2
    slider1_velocity = .-scale_v .* (cos.(bar_angle_vector) .- sin.(bar_angle_vector))
    slider2_velocity =  scale_v .* (sin.(bar_angle_vector) .+ cos.(bar_angle_vector))

    scale_a = bar_length * bar_angular_rate^2 / √2
    slider1_accel =  scale_a .* (sin.(bar_angle_vector) .+ cos.(bar_angle_vector))
    slider2_accel =  scale_a .* (cos.(bar_angle_vector) .- sin.(bar_angle_vector))

    position_plot =
        plot(time_vector, [slider1_position slider2_position],
             xlabel="t (s)", ylabel="s (m)",
             label=[L"s_1(t)" L"s_2(t)"],
             title="Slider Positions")

    velocity_plot =
        plot(time_vector, [slider1_velocity slider2_velocity],
             xlabel="t (s)", ylabel=L"\dot{s}\,(m/s)",
             label=[L"\dot{s}_1(t)" L"\dot{s}_2(t)"],
             title="Slider Velocities")

    acceleration_plot =
        plot(time_vector, [slider1_accel slider2_accel],
             xlabel="t (s)", ylabel=L"\ddot{s}\,(m/s^2)",
             label=[L"\ddot{s}_1(t)" L"\ddot{s}_2(t)"],
             title="Slider Accelerations")
	
	plot(position_plot, velocity_plot, acceleration_plot,layout=(3,1), size=(900,900))
end
#\Reece Moczek 4/4/26
...
begin
    ...
    animation_object = @animate for k in eachindex(time_vector)
        ...
        frame = plot(track_x, track_x,
                     ...
                     aspect_ratio=:equal,
                     xlims=(-track_extent, track_extent),
                     ylims=(-track_extent, track_extent),
                     title="t = $(round(time_vector[k],digits=2)) s")
...
manifest_format = "2.0"
project_hash = "49b39572d670a48c5ef2dc11f64ffe7422b5275f"
